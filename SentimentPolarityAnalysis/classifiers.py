...
def analyse_sentence(sentence, print_show=False):
    # 情感分析整体数据结构
    comment_analysis = {"score": 0}
    # 将评论分句
    the_clauses = __divide_sentence_into_clauses(sentence + "%")
    # 对每分句进行情感分析
    for i in range(len(the_clauses)):
        # 情感分析子句的数据结构
        sub_clause = __analyse_clause(the_clauses[i].replace("。", "."), print_show)
        # 将子句分析的数据结果添加到整体数据结构中
        comment_analysis["su-clause" + str(i)] = sub_clause
        comment_analysis['score'] += sub_clause['score']
    if print_show:
        print("\n" + sentence)
        __output_analysis(comment_analysis)
        print(comment_analysis)
    return comment_analysis["score"]


def __analyse_clause(the_clause, print_show):
    ...
    if print_show:
        ...
        print(seg_result)
    # 判断句式：如果……就好了
    judgement = __is_clause_pattern2(the_clause)
    if judgement != "":
        sub_clause["pattern"].append(judgement)
        sub_clause["score"] -= judgement["value"]
        return sub_clause
    # 判断句式：是…不是…
    judgement = __is_clause_pattern1(the_clause)
    if judgement != "":
        sub_clause["pattern"].append(judgement)
        sub_clause["score"] -= judgement["value"]
    # 判断句式：短语
    judgement = __is_clause_pattern3(the_clause, seg_result)
    if judgement != "":
    ...
    for i in range(len(seg_result)):
        mark, result = __analyse_word(seg_result[i].word, seg_result, i)
        if mark == 0:
            continue
        elif mark == 1:
...
def __is_clause_pattern3(the_clause, seg_result):
    for a_phrase in __phrase_dict:
        keys = a_phrase.keys()
                    ...
                    if seg_result[i].word in match.group():
                        try:
                            if seg_result[i + 1].word in match.group():
                                return __emotional_word_analysis(
...
def __analyse_word(the_word, seg_result=None, index=-1):
    # 判断是否是连词
    judgement = __is_word_conjunction(the_word)
    if judgement != "":
        return 1, judgement

    # 判断是否是标点符号
    judgement = __is_word_punctuation(the_word)
    if judgement != "":
        return 2, judgement

    # 判断是否是正向情感词
    judgement =__is_word_positive(the_word, seg_result, index)
    if judgement != "":
        return 3, judgement

    # 判断是否是负向情感词
    judgement = __is_word_negative(the_word, seg_result, index)
    if judgement != "":
        return 4, judgement
...
def __is_word_conjunction(the_word):
    if the_word in __conjunction_dict:
        conjunction = {"key": the_word, "value": __conjunction_dict[the_word]}
        return conjunction
    return ""


def __is_word_punctuation(the_word):
    if the_word in __punctuation_dict:
        punctuation = {"key": the_word, "value":__punctuation_dict[the_word]}
        return punctuation
    return ""


def __is_word_positive(the_word, seg_result, index):
    # 判断分词是否在情感词典内
    if the_word in __positive_dict:
        # 在情感词典内，则构建一个以情感词为中心的字典数据结构
        return __emotional_word_analysis(the_word, __positive_dict[the_word],
                                              [x for x, y in seg_result], index)
    # 不在情感词典内，则返回空
    return ""


def __is_word_negative(the_word, seg_result, index):
    # 判断分词是否在情感词典内
    if the_word in __negative_dict:
        # 在情感词典内，则构建一个以情感词为中心的字典数据结构
        return __emotional_word_analysis(the_word, __negative_dict[the_word],
                                              [x for x, y in seg_result], index)
    # 不在情感词典内，则返回空
    return ""


def __emotional_word_analysis(core_word, value, segments, index):
    # 在情感词典内，则构建一个以情感词为中心的字典数据结构
    ...
    if view_window > -1:  # 无越界
        # 判断前一个词是否是情感词
        if segments[view_window] in __negative_dict or \
                segments[view_window] in __positive_dict:
            orientation['score'] = orientation_score
            return orientation
        # 判断是否是副词
        if segments[view_window] in __adverb_dict:
            # 构建副词字典数据结构
            adverb = {"key": segments[view_window], "position": 1,
                      "value": __adverb_dict[segments[view_window]]}
            orientation["adverb"].append(adverb)
            orientation_score *= __adverb_dict[segments[view_window]]
        # 判断是否是否定词
        elif segments[view_window] in __denial_dict:
            # 构建否定词字典数据结构
            denial = {"key": segments[view_window], "position": 1,
                      "value": __denial_dict[segments[view_window]]}
    ...
    if view_window > -1:
        # 判断前一个词是否是情感词
        if segments[view_window] in __negative_dict or segments[view_window] in __positive_dict:
            orientation['score'] = orientation_score
            return orientation
        if segments[view_window] in __adverb_dict:
...
def __output_analysis(comment_analysis):
...
def __divide_sentence_into_clauses(the_sentence):
    the_clauses = __split_sentence(the_sentence)

    # 识别“是……不是……”句式
    pattern = re.compile(r"([，、。%！；？?,!～~.… ]*)([\u4e00-\u9fa5]*?(要|选)"
                         r"的.+(送|给)[\u4e00-\u9fa5]+?[，。！%；、？?,!～~.… ]+)")
    match = re.search(pattern, the_sentence.strip())
    if match is not None and len(__split_sentence(match.group(2))) <= 2:
        to_delete = []
        for i in range(len(the_clauses)):
    ...
    pattern = re.compile(r"([，%。、！；？?,!～~.… ]*)([\u4e00-\u9fa5]*?(如果|要是|"
                         r"希望).+就[\u4e00-\u9fa5]+(好|完美)了[，。；！%、？?,!～~.… ]+)")
    match = re.search(pattern, the_sentence.strip())
    if match is not None and len(__split_sentence(match.group(2))) <= 3:
        to_delete = []
        for i in range(len(the_clauses)):
            if the_clauses[i] in match.group(2):
                to_delete.append(i)
...
def __get_phrase_dict():
    sentiment_dict = []
    pattern = re.compile(r"\s+")
    with open(__root_filepath + "phrase_dict.txt", "r", encoding="utf-8") as f:
        for line in f:
            a_phrase = {}
...
if __name__ == '__main__':
    a_sentence = sys.argv[1]
    # a_sentence = "好大味儿"
    __root_filepath = "E:\\BarrageProject\\SentimentPolarityAnalysis\\f_dict\\"
    jieba.load_userdict(__root_filepath+"user.dict")  # 准备分词词典
    __phrase_dict = __get_phrase_dict()
    __positive_dict = __get_dict(__root_filepath + "positive.txt")
    __negative_dict = __get_dict(__root_filepath + "negative.txt")
    __conjunction_dict = __get_dict(__root_filepath + "conjunction_dict.txt")
    __punctuation_dict = __get_dict(__root_filepath + "punctuation_dict.txt")
    __adverb_dict = __get_dict(__root_filepath + "adverb_dict.txt")
    __denial_dict = __get_dict(__root_filepath + "denial_dict.txt")
    result = analyse_sentence(a_sentence, True)
    # print(a_sentence)
    print(result)

# 调用：python classifiers.py "哈哈哈或或或或或"
